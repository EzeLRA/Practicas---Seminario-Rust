...
impl Informe{
    //fn procesar_venta(&mut self,v:Venta); //Agrega la venta con la informacion necesaria para el informe
    //fn ordenar_cronologia(&mut self); Ordena de menor a mayor para cada espacio dia/mes/anio
}

impl Sistema {
    //Se utiliza el dni del cliente como criterio principal para el algoritmo
    fn get_historial_compras(&mut self,id:u64,monto_minimo:f64)->Option<Informe>{
        let mut res : Option<Informe> = None;
        
        /*
            Implementar:
                +Recorrido sobre las ventas (Si dispone el sistema o no , para dicho caso concluye el metodo en None)
                +Filtrar solo las ventas que contengan el dni del cliente y cumplan con la condicion del monto final
                    + Se reutiliza la funcion "monto final venta" para calcular la venta en esta funcion

                +Finalizado el proceso se debera ordenar el informe antes de retornar(Si es que se consiguio alguna venta en el anterior proceso , Caso contrario retorna None)
                    +El informe se ordenara por cronologia : res.ordenar_cronologia();
        */
